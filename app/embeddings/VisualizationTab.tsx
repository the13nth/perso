...
export default function VisualizationTab({ documents, notes, activities }: VisualizationTabProps) {
  ...
  useEffect(() => {
    if (!plotContainer || points.length === 0) return;

    console.log('[Visualization] Rendering plot with:', {
      containerWidth: plotContainer.offsetWidth,
    ...
    console.log('[Visualization] Filtered points:', {
      numFiltered: filteredPoints.length,
      types: new Set(filteredPoints.map(p => p.type))
    });
    ...
    try {
      console.log('[Visualization] Calling Plotly.newPlot');
      Plotly.newPlot(plotContainer, plotData, layout, {
        displayModeBar: true,
        responsive: true,
      });
      console.log('[Visualization] Plot rendered successfully');
    } catch (error) {
      console.error('[Visualization] Error rendering plot:', error);
    }
    ...
    return () => {
      if (plotContainer) {
        try {
          console.log('[Visualization] Cleaning up plot');
          Plotly.purge(plotContainer);
        } catch (error) {
          console.error('[Visualization] Error cleaning up plot:', error);
        }
      }
    };
  }, [points, selectedTypes, plotContainer]);
